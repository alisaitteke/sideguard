...
func TestRunLLMTestWithMockClassifier(t *testing.T) {
	...
	sideguardDir := home + "/.sideguard"
	if err := writeTestFile(sideguardDir+"/config.yaml", `llm:
  enabled: true
...
`); err != nil {
	...
	if err := writeTestFile(sideguardDir+"/credentials.yaml", `providers:
  my-openai:
    api_key: sk-test
`); err != nil {
		t.Fatal(err)
	}
	if err := writeTestFile(sideguardDir+"/signatures/default.yaml", `name: default
system: test
`); err != nil {
		t.Fatal(err)
	}
...
func TestEvaluateLLMTestYAMLAllowSkipsLLM(t *testing.T) {
	...
	t.Setenv("HOME", home)
	sideguardDir := home + "/.sideguard"
	if err := writeTestFile(sideguardDir+"/policy.yaml", `rules:
