...
var llmProviderCmd = &cobra.Command{
	Use:   "provider",
	Short: "Manage LLM provider instances in ~/.sideguard",
...
func runProviderRemove(_ *cobra.Command, _ []string) error {
	...
	if settings.DefaultProvider == id {
		return fmt.Errorf("cannot remove default provider %q; set another default first with `sideguard llm provider set-default`", id)
	}
