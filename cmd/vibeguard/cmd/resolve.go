...
func handlePendingResolveError(err error, verb string) error {
	...
	if errors.Is(err, ErrNoPendingApprovals) {
		...
		fmt.Println()
		fmt.Println("Hint: run `vibeguard ui` to watch for new requests.")
		return nil
	}
	var multi *MultiplePendingError
	if errors.As(err, &multi) {
		fmt.Fprintf(os.Stderr, "Multiple pending approvals — run `vibeguard ui` or `vibeguard %s <id>`\n\n", verb)
...
func runRootDefault() error {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "hint: start the daemon with `vibeguard daemon start`\n")
	...
	fmt.Println()
	fmt.Println("Interactive approvals: vibeguard ui")
	fmt.Println("List queue:          vibeguard pending")
	fmt.Println("Quick approve/deny:  vibeguard approve  |  vibeguard deny  (when one pending)")
	return nil
}
