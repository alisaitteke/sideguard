...
var installCmd = &cobra.Command{
	...
	Long:  "Discovers client configs, backs them up, wraps MCP servers with vibeguard wrap, merges hooks, installs the daemon LaunchAgent, and on macOS registers the menu-bar tray LaunchAgent (use --headless to skip tray).",
...
var uninstallCmd = &cobra.Command{
	Use:   "uninstall",
	Short: "Remove VibeGuard hooks, MCP wraps, and daemon LaunchAgent",
	Long:  "Default: surgically removes VibeGuard hook entries and unwraps MCP servers in-place (preserves your hooks), and removes daemon and tray LaunchAgents on macOS. Use --restore-backup to restore the oldest pre-install backup per file instead.",
	RunE: func(_ *cobra.Command, _ []string) error {
...
func init() {
	...
	installCmd.Flags().BoolVar(&installDev, "dev", false, "write repo-scoped workspace dev policy (.vibeguard/policy.yaml) for make/go/scripts")
