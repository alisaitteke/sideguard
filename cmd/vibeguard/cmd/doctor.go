...
import (
	...
	"github.com/alisaitteke/vibeguard/internal/doctor"
)
...
var doctorCmd = &cobra.Command{
	Use:   "doctor",
	Short: "Diagnose VibeGuard install health and bypass vectors",
...
Reports HIGH when hooks are missing (bypass risk) or the daemon is down.`,
	RunE: func(_ *cobra.Command, _ []string) error {
		...
		if report.HasHigh() {
			fmt.Fprintln(os.Stderr, "\nVibeGuard doctor: HIGH severity findings — run `vibeguard install` to repair")
		} else {
			fmt.Println("\nVibeGuard doctor: no HIGH severity findings")
		}
