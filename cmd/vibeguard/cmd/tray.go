...
var trayCmd = &cobra.Command{
	...
	Long: `Run VibeGuard in the menu bar as a background process.
...
icon (not a context menu). The panel polls the daemon every ~2s and shows:
  ...
  • Use "vibeguard ui" for more than 10 pending items
...
Start the daemon first (vibeguard daemon start), then run this command.
Use "vibeguard mode set auto-allow" or the tray mode control for hands-off local dev.

Requires CGO_ENABLED=1 at build time and an active GUI session (not SSH-only).

On macOS you can also run dist/VibeGuard Tray.app (make tray-app) or install a
LaunchAgent: vibeguard tray install-service`,
	RunE: runTray,
}
...
var trayInstallServiceCmd = &cobra.Command{
	...
	Long: `Writes ~/Library/LaunchAgents/com.vibeguard.tray.plist and loads it via launchctl.

The tray starts at login (RunAtLoad) but does not respawn when quit (KeepAlive false).
Start the daemon separately (vibeguard daemon start or daemon install-service).

Uses the current vibeguard binary path at install time.`,
