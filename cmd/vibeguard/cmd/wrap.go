...
import (
	...
	"github.com/alisaitteke/vibeguard/internal/proxy"
)
...
Example:
  vibeguard wrap -- npx -y @modelcontextprotocol/server-filesystem /tmp`,
...
func parseWrapArgs(args []string) ([]string, error) {
	...
	if sep < 0 {
		return nil, fmt.Errorf("usage: vibeguard wrap -- <upstream-command> [args...]")
	}
	upstream := args[sep+1:]
	if len(upstream) == 0 {
